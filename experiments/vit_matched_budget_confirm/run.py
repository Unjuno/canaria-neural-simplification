...
def run(seed,out):
 ...
 if seed not in P["fresh_seeds"]: raise ValueError("seed outside locked plan")
 ...
 pn,dn=vit.count_params(pair),vit.count_params(direct)
 if pn!=9808 or dn!=9844: raise RuntimeError("parameter invariant")
 with torch.no_grad(): pe=vit.nmse(pair(Xh),Yh)[0]; de=vit.nmse(direct(Xh),Yh)[0]
 r={"seed":seed,"eligible":bva>=.95,"teacher_val_acc":bva,"component_pair8":{"parameters":pn,"hold_nmse":pe,"val_acc":valacc(base,pair,True,va)},"direct_single84":{"parameters":dn,"hold_nmse":de,"val_acc":valacc(base,direct,False,va)},"test_used":False,"elapsed_not_benchmark":time.time()-started}
