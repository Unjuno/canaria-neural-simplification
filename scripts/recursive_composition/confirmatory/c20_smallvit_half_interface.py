...
BASIS_SPECS = [
    ("identity_first16", None),
    ("random_20261101", 20261101),
    ("random_20261102", 20261102),
    ("random_20261103", 20261103),
]
...
def basis_projection(seed):
    if seed is None:
        return torch.eye(32, dtype=torch.float32)[:, :16]
...
def run(seed: int):
    set_seed(seed)
    X, y, tr_idx, va_idx, te_idx = split_indices_and_arrays()
    tr = make_ds(X, y, tr_idx)
    va = make_ds(X, y, va_idx)
    ...
    result = {
        ...
        "test_evaluated_after_validation_only": False,
    }
    ...
    frozen_cluster = copy.deepcopy(base)
    frozen_final, frozen = compile_final(teacher, frozen_cluster, a0fit, a0v, a2v, denom, va, final_init, final_fit)
    final_models["frozen"] = copy.deepcopy(frozen_final)

    full_cluster = copy.deepcopy(base)
    adapt_full(full_cluster, a0fit, a2fit, align_seed)
    full_final, full = compile_final(teacher, full_cluster, a0fit, a0v, a2v, denom, va, final_init, final_fit)
    final_models["full_32"] = copy.deepcopy(full_final)

    basis_records = {}
    for basis_name, basis_seed in BASIS_SPECS:
        p16 = basis_projection(basis_seed)

        sketch_cluster = copy.deepcopy(base)
        adapt_sketch(sketch_cluster, a0fit, a2fit, p16, align_seed)
        sketch_final, sketch = compile_final(teacher, sketch_cluster, a0fit, a0v, a2v, denom, va, final_init, final_fit)
        final_models[f"sketch::{basis_name}"] = copy.deepcopy(sketch_final)

        anchored_cluster = copy.deepcopy(base)
        with torch.no_grad():
            correction = ((a2fit - baseline_fit) @ p16) @ p16.T
            anchored_target = (baseline_fit + correction).detach()
        adapt_full(anchored_cluster, a0fit, anchored_target, align_seed)
        anchored_final, anchored = compile_final(teacher, anchored_cluster, a0fit, a0v, a2v, denom, va, final_init, final_fit)
        final_models[f"anchored::{basis_name}"] = copy.deepcopy(anchored_final)

        basis_records[basis_name] = {
            "basis_seed": basis_seed,
            "sketch_only_16": sketch,
            "anchored_16": anchored,
    ...
    anchored_nmse = {k: v["anchored_16"]["final_nmse_vs_original"] for k, v in basis_records.items()}
    worst_basis = max(anchored_nmse, key=anchored_nmse.get)
    best_basis = min(anchored_nmse, key=anchored_nmse.get)
    anchored_acc = {k: v["anchored_16"]["final_replacement_val_acc"] for k, v in basis_records.items()}
    min_acc_basis = min(anchored_acc, key=anchored_acc.get)

    # Test dataset is constructed only after all fitting and validation metrics above are complete.
    te = make_ds(X, y, te_idx)
    full_test_acc = accuracy(ReplacedViT(teacher, copy.deepcopy(final_models["full_32"]), start=1), te)
    anchored_test_acc = {}
    for basis_name, _ in BASIS_SPECS:
        anchored_test_acc[basis_name] = accuracy(
            ReplacedViT(teacher, copy.deepcopy(final_models[f"anchored::{basis_name}"]), start=1), te
        )
    min_test_basis = min(anchored_test_acc, key=anchored_test_acc.get)
    ...
    result.update({
        ...
        "aggregate": {
            ...
            "min_validation_accuracy_basis": min_acc_basis,
            "min_test_accuracy_basis": min_test_basis,
            ...
            "validation_accuracy_difference_worst": anchored_acc[min_acc_basis] - full["final_replacement_val_acc"],
            "test_accuracy_difference_worst": anchored_test_acc[min_test_basis] - full_test_acc,
            "full_32_test_acc": full_test_acc,
            "anchored_16_test_acc_by_basis": anchored_test_acc,
        },
