...
FRESH_SEEDS = tuple(range(65400, 65416))
IMPLEMENTATION_VERIFICATION_SEED = 65300
SIGMAS = (0.20, 0.28, 0.36, 0.44, 0.52, 0.60)
CALIBRATION_SAMPLES = 192
FIT_UPDATES = 600
CAL_EPS_OFFSET = 670100
VAL_EPS_OFFSET = 670200
TOP_ADAPTATION_OFFSET = 675000
FINAL_INIT_OFFSET = 676000
FINAL_FIT_OFFSET = 677000
...
def run(seed: int, allow_verification_seed: bool = False) -> dict:
    ...
    for sigma in SIGMAS:
        ...
        teacher_shifted_acc = accuracy(teacher, Xv_shift, yv)
        with torch.no_grad():
            correction_v = correction_for_k(residual_v, q, 2)
        mechanism = {
            "teacher_shifted_validation_accuracy": teacher_shifted_acc,
            "teacher_accuracy_drop_from_clean_pp": 100.0 * (teacher_shifted_acc - teacher_clean_acc),
            "p2_accuracy_gap_to_shifted_teacher_pp": 100.0 * (
                p2["final_replacement_val_acc"] - teacher_shifted_acc
            ),
