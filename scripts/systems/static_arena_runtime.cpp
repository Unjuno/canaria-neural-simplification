...
static constexpr size_t ARENA_BYTES=65536;
...
static void run_block(float* x,float* z,float* k,float* v,float* score,float* q,float* ctx,float* out,float* ln,float* hid,const W& w){
    for(int r=0;r<B*T;++r) layernorm_row(x+r*D,w.n1w,w.n1b,z+r*D);
    const float* wk=w.ipw+D*D; const float* wv=w.ipw+2*D*D; const float* bk=w.ipb+D; const float* bv=w.ipb+2*D;
    for(int r=0;r<B*T;++r){ linear24(z+r*D,wk,bk,k+r*D); linear24(z+r*D,wv,bv,v+r*D); }
    const float* wq=w.ipw; const float* bq=w.ipb;
    const float scale=1.f/std::sqrt((float)HD);
    for(int b=0;b<B;++b){
        for(int ti=0;ti<T;++ti){
            linear24(z+(b*T+ti)*D,wq,bq,q);
            for(int h=0;h<H;++h){
                float mx=-INFINITY;
                for(int tj=0;tj<=ti;++tj){ float s=0.f; for(int d=0;d<HD;++d)s+=q[h*HD+d]*k[(b*T+tj)*D+h*HD+d]; s*=scale; score[h*T+tj]=s; if(s>mx)mx=s; }
                float den=0.f; for(int tj=0;tj<=ti;++tj){ float e=std::exp(score[h*T+tj]-mx); score[h*T+tj]=e; den+=e; }
                for(int d=0;d<HD;++d){ float c=0.f; for(int tj=0;tj<=ti;++tj)c+=(score[h*T+tj]/den)*v[(b*T+tj)*D+h*HD+d]; ctx[h*HD+d]=c; }
            }
            linear24(ctx,w.opw,w.opb,out);
            float* xr=x+(b*T+ti)*D; for(int d=0;d<D;++d) xr[d]+=out[d];
        }
    }
    const float invsqrt2=0.7071067811865475244f;
    for(int r=0;r<B*T;++r){
        float* xr=x+r*D; layernorm_row(xr,w.n2w,w.n2b,ln);
        linear24(ln,w.m0w,w.m0b,hid); for(int j=0;j<D;++j) hid[j]=0.5f*hid[j]*(1.f+::erff(hid[j]*invsqrt2));
        linear24(hid,w.m2w,w.m2b,out); for(int j=0;j<D;++j) xr[j]+=out[j];
    }
}
...
int main(){
    ...
    float* z=(float*)arena.alloc(ACT_FLOATS*4);
    ...
    float* ref=(float*)arena.alloc(ACT_FLOATS*4);
    if(!weights||!x||!z||!k||!v||!score||!q||!ctx||!out||!ln||!hid||!ref) return 90;
    g_guard=1;
    ...
    for(int bi=0;bi<2;++bi){
        ...
        W w=parse(weights); run_block(x,z,k,v,score,q,ctx,out,ln,hid,w);
    }
    clock_gettime(CLOCK_MONOTONIC,&t1);
    if(!read_exact("/data/reference.bin",ref,ACT_FLOATS*4)) return 93;
    double dsq=0.,rsq=0.,osum=0.; float maxabs=0.f;
    for(size_t i=0;i<ACT_FLOATS;++i){ float d=x[i]-ref[i]; float a=std::fabs(d); if(a>maxabs)maxabs=a; dsq+=(double)d*d; rsq+=(double)ref[i]*ref[i]; osum+=x[i]; }
    double rel=std::sqrt(dsq)/(std::sqrt(rsq)+1e-30);
    g_guard=0;
    bool sums_ok=std::fabs(bsum[0]-EXPECTED_BLOCK0_SUM)<1e-4f && std::fabs(bsum[1]-EXPECTED_BLOCK1_SUM)<1e-4f;
    bool pass=maxabs<=5e-5f && rel<=1e-5 && arena.high<=ARENA_BYTES && g_heap_violations==0 && sums_ok;
    char buf[2048];
    int n=snprintf(buf,sizeof(buf),"{\"status\":\"%s\",\"max_abs\":%.9g,\"relative_l2\":%.12g,\"output_sum\":%.12g,\"arena_high_water_bytes\":%zu,\"arena_capacity_bytes\":%zu,\"heap_allocation_violations\":%llu,\"block0_sum\":%.9g,\"block1_sum\":%.9g,\"block_sums_ok\":%s,\"full_score_tensor_created\":false,\"elapsed_seconds\":%.9g}\n",pass?"PASS":"FAIL",maxabs,rel,osum,arena.high,ARENA_BYTES,(unsigned long long)g_heap_violations,bsum[0],bsum[1],sums_ok?"true":"false",secs(t0,t1));
